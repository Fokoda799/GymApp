...
export default function BodyParts() {
  ...
  const load = useCallback(async () => {
    try {
      const r = await axios.get("/api/bodyparts", { params: { page, limit: LIMIT, search } })
      setData(r.data.data)
  ...
  function openEdit(item) { setForm(item); setError(""); setModal("edit") }
  ...
  async function handleSave(e) {
    ...
    setError("")
    try {
      if (modal === "create") await axios.post("/api/bodyparts", form)
      else await axios.put(`/api/bodyparts/${form.id}`, form)
      setModal(null)
      load()
    } catch (err) {
  ...
  async function handleDelete() {
    try {
      await axios.delete(`/api/bodyparts/${delTarget}`)
      setDelTarget(null)
      load()
  ...
  return (
    <Layout>
      <div className="flex items-center justify-between mb-6 gap-4 flex-wrap">
        <div>
          <h1 className="text-white text-2xl font-bold">Body Parts</h1>
      ...
      <div className="mb-4">
        <input type="text" placeholder="Search body parts..." value={search}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
        {data.length === 0 ? (
          <div className="col-span-full text-center text-muted text-sm py-12">No body parts found</div>
        ) : data.map(item => (
      ...
      {(modal === "create" || modal === "edit") && (
        <Modal title={modal === "create" ? "New Body Part" : "Edit Body Part"} onClose={() => setModal(null)}>
      ...
      {delTarget && <ConfirmDelete label="body part" onConfirm={handleDelete} onCancel={() => setDelTarget(null)} />}
    </Layout>
  )
}
